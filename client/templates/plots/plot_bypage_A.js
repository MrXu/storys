 ...
 */
Template.plot_bypage_A.created = function () {
    this.autorun(function () {
        this.subscription = Meteor.subscribe('plotCompo', Router.current().params._id);
    }.bind(this));
};

Template.plot_bypage_A.rendered = function () {
    this.autorun(function () {
...
Template.plot_bypage_A.helpers({
    plot: function(){
    ...
    story: function(){
        var plot = Plots.findOne({_id: Router.current().params._id});
        if(plot){
            return Storys.findOne({_id:plot.storyId});
        }
    }
